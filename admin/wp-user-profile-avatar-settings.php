...
/**
 * WPEM_User_Profile_Avatar_Settings class.
 */

class WPEM_User_Profile_Avatar_Settings {
	...
	public function settings() 
	{
		...
		$wp_user_profile_avatar_attachment_url = wpem_get_default_avatar_url(['size' => 'admin']);
		?>
		...
		<div class="wrap">
		  	...
		  	<table>
		  		<tr valign="top">
		  			<td>
			  			<form method="post" action="<?php echo admin_url('users.php'). '?page=wpem-profile-avatar-settings'; ?>">
				  			...
				  			<table class="form-table">
			  					...
			  					<tr valign="top">
			  						<th scope="row"><?php _e('Avatar Display', 'wp-user-profile-avatar'); ?></th>
			  						<td>
			  							<fieldset>
							              <label for="wp_user_profile_avatar_show_avatars">
							                <input name="wp_user_profile_avatar_show_avatars" type="checkbox" id="wp_user_profile_avatar_show_avatars" value="1" <?php echo checked($wp_user_profile_avatar_show_avatars, 1, 0); ?> > <?php _e('Disable Gravatar and use only local avatars', 'wp-user-profile-avatar'); ?>
							              </label>
							            </fieldset>
			  						</td>
			  					</tr>
			  					...
			  					<tr valign="top">
			  						...
			  						<td>
			  							<fieldset>
							              	...
							              	<?php foreach (wpem_get_avatar_rating() as $name => $rating) : ?>
			  					...
			  					<tr valign="top">
			  						...
			  						<td class="defaultavatarpicker">
			  							<fieldset>
							              	...
							              	<label><input type="radio" name="wp_user_profile_avatar_default" id="wp_user_profile_avatar_radio" value="wp_user_profile_avatar" <?php echo $selected; ?> /> <div id="wpem-preview"><img src="<?php echo $wp_user_profile_avatar_attachment_url; ?>" width="32" /></div> <?php _e('WP User Profile Avatar'); ?> </label><br />
							              	...
							              	?>
							              	<p id="wpem-edit">
							              		<button type="button" class="button" id="wpem-add" name="wpem-add"><?php _e('Choose Image'); ?></button>
							              		<span id="wpem-remove-button" class="<?php echo $class_hide; ?>"><a href="javascript:void(0)" id="wpem-remove"><?php _e('Remove'); ?></a></span>
							              		<span id="wpem-undo-button"><a href="javascript:void(0)" id="wpem-undo"><?php _e('Undo'); ?></a></span>
							              	...
							              	<?php foreach (wpem_get_default_avatar() as $name => $label) : ?>
...
new WPEM_User_Profile_Avatar_Settings();