...
if( ! defined( 'ABSPATH' ) ) {
 ...
 use \WpUserNameChange\WpUserNameChange;
	function Wp_username_edit() { ?>
		...
		<p><h1><?php _e('Wp Users List','WP_Username_change') ?></h1></p>
		<?php
		    ...
		    if($records) {
		        ...
		        <table class="wp-list-table widefat fixed striped users"  cellpadding="3" cellspacing="3" width="100%">
		             <thead>
			          <tr>
			            <th><strong><?php _e('User ID','WP_Username_change') ?></strong></th>
			            <th><strong><?php _e('User Name','WP_Username_change') ?></strong></th>
			            <th><strong><?php _e('Role','WP_Username_change') ?></strong></th>
			            <th><strong><?php _e('Update','WP_Username_change') ?></strong></th>
			          </tr>
			         </thead>
			         <tbody>
					    <?php
