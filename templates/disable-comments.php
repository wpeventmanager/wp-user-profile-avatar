...
function comments_settings_page() {
    include dirname(__FILE__) . '/comments-settings-page.php';
}
...
function comments_tools_page() {
    include dirname(__FILE__). '/comments-tools-page.php';
}
...
function check_comment_template() {
    $options = get_option( 'disable_comments_options', array() );
    if ( is_singular() && ( $options['remove_everywhere'] || is_post_type_disabled( get_post_type() ) ) ) {
...
function get_disabled_post_types() {
    $options = get_option( 'disable_comments_options', array() );
    $types = $options['disabled_post_types'];
    // Not all extra_post_types might be registered on this particular site.
        foreach ( (array) $options['extra_post_types'] as $extra ) {
